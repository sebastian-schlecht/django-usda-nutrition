...
class FoodDescription(models.Model):
    ...
    survey = models.NullBooleanField(null=True, blank=True, help_text='Indicates if the food item is used in the USDA Food and Nutrient Database for Dietary Studies (FNDDS) and thus has a complete nutrient profile for the 65 FNDDS nutrients.')
...
class NutrientData(models.Model):
    ...
    ref_food_description = models.ForeignKey(FoodDescription, related_name='ref_nutrient_data', blank=True, null=True, help_text='NDB number of the item used to calculate a missing value. Populated only for items added or updated starting with SR14.')
    add_nutr_mark = models.NullBooleanField(null=True, blank=True, help_text='Indicates a vitamin or mineral added for fortification or enrichment. This field is populated for ready-to- eat breakfast cereals and many brand-name hot cereals in food group 08.')
    num_studies = models.PositiveSmallIntegerField(null=True, blank=True, help_text='Number of studies.')
    minimum = models.DecimalField(max_digits=13, decimal_places=3, null=True, help_text='Minimum value.')
    maximum = models.DecimalField(max_digits=13, decimal_places=3, null=True, help_text='Maximum value.')
    degrees_of_freedom = models.PositiveSmallIntegerField(null=True, blank=True, help_text='Degrees of freedom.')
    lower_error_bound = models.DecimalField(max_digits=13, decimal_places=3, null=True, help_text='Lower 95% error bound.')
    upper_error_bound = models.DecimalField(max_digits=13, decimal_places=3, null=True, help_text='Upper 95% error bound.')
